...
The author runs these in a clean WoT 2.3.1.1 with **only** SpotMeter **plus its
...
- [ ] **Loads**: `python.log` has `SpotMeter: initialised (version=7.1.0, ...)`
...
1. Move the `CHANGELOG.md` `[7.1.0]` entry from *Unreleased* to a dated release.
2. Commit (working tree clean → re-run preflight to confirm).
3. `git tag v7.1.0` and push the tag.
4. Build the final artifact from the tagged tree; create the GitHub release and
   attach `dist/spotmeter-v7.1.0.wotmod` (+ the `.zip` bundle).
